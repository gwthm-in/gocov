...
- The tool is built with Go 1.25.0 (minimum required version)
- A single binary works with all Go 1.25.0+ installations - **no separate releases needed for different Go versions**
- The tool works with coverage profiles generated by Go 1.25.0 and later
- The tool calls `go test` and `go tool cover` from the user's installed Go version
- We test compatibility with Go 1.25.0 and 1.26.0+ in CI, but only build one set of binaries
